...
function ManageAddressScreen(props) {
    const [state, setState] = React.useState({ addresses: [], showAdd: false, countries: [], sumAmount: 0, selectedAddress: null, showEdit: false, editAddressData: [], step: 1, selectedPaymentMethod: 4, paymentSuccessModal: false, loading: true, showAlert: false });
    const [showMessage, setShowLoading] = React.useState(false)
    const [messageState, setMessage] = React.useState({ message: null, type: null })
    const { message, type } = messageState;
    const { showAdd, addresses, selectedAddress, showEdit, editAddressData, step, countries, loading, showAlert } = state;
    ...
    const editAddress = (data) => {
        ...
        setState({ ...state, showEdit: true, editAddressData: data, selectedAddress: data.id });
    }
