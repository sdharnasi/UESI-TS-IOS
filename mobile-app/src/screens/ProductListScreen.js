...
function ProductListScreen(props) {
    ...
    const [titleHeading, setHeading] = React.useState(null);

    const fetchData = (ID, page, filterApplied = false, range, fprice = null, frating = null) => {

        let url = '';
        if (type == 'category' || type == 'categorybanner') {
            url = 'getProductByCategory/' + ID + '?page=' + page
        }
        else if (type == 'menufacturer' || type == 'brandbanner') {
            ...
            getApi.getData(
                ...
                if (response.status == 1) {
                    ...
                    if (type == 'categorybanner') {
                        setHeading(response.categoryData?.category_description?.name)
                    }
                    if (type == 'brandbanner') {
                        setHeading(response.brandData?.name)
                    }
        ...
        <OtrixContainer customStyles={{ backgroundColor: Colors().light_white }}>
            ...
            <OtrixHeader customStyles={{ backgroundColor: Colors().light_white }}>
                ...
                <View style={[GlobalStyles.headerCenter]}>
                    {
                        type == 'categorybanner' || type == 'brandbanner' ? <Text style={GlobalStyles.headingTxt}>{titleHeading}</Text> :
